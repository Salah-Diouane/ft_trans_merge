...
const Game = () => {
	...
	useEffect(() => {
		const setgameinfo = async () => {
			const response = await fetch("https://e3r5p2.1337.ma/api/settings/gameinfo", {
				credentials: 'include'
			});
			if (response.ok) {
		...
		const setTicTacInfo = async () => {
			const response = await fetch("https://e3r5p2.1337.ma/api/settings/tictacinfo", {
				credentials: 'include'
			});
			if (response.ok) {
	...
	const sendData = async (e: any) => {
		...
		const response = await fetch("https://e3r5p2.1337.ma/api/settings/game", {
			method: 'PUT',
			credentials: "include",
	...
	const sendTicTacData = async (e: any) => {
		...
		const response = await fetch("https://e3r5p2.1337.ma/api/settings/tictac", {
			method: 'PUT',
			credentials: "include",
			headers: { 'Content-type': 'application/json' },
			body: JSON.stringify(body),

...
const FriendList: React.FC<{ status: FriendCardProps["status"] }> = ({ status }) => {
  ...
  useEffect(() => {
    ...
    const fetchRequest = async () => {
      const res = await fetch("http://e3r11p10.1337.ma:3000/friends/allsendreq", { credentials: "include" });
      const data = await res.json();
      setFriends(Array.isArray(data) ? data : []);
    };

    const myAllfriends = async () => {
      const res = await fetch("http://e3r11p10.1337.ma:3000/friends/allfriends", { credentials: "include" });
      const data = await res.json();
      setAllUsers(data);
    };

    const allRecvReq = async () => {
      const res = await fetch("http://e3r11p10.1337.ma:3000/friends/allrecvreq", { credentials: "include" });
      const data = await res.json();
      setAllreqs(Array.isArray(data) ? data : []);
    };

    const allBlocked = async () => {
      const res = await fetch("http://e3r11p10.1337.ma:3000/friends/blockReq", { credentials: "include" });
      const data = await res.json();
  ...
  const handleAccept = async (username: string) => {
    ...
    await fetch("http://e3r11p10.1337.ma:3000/friends/acceptrequest", {
  ...
  const handleReject = async (username: string) => {
    ...
    await fetch("http://e3r11p10.1337.ma:3000/friends/deletereq", {
  ...
  const unblockUser = async (username: string) => {
    ...
    await fetch("http://e3r11p10.1337.ma:3000/friends/unblockUser", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ frined_username: username, type: "send" }),
      credentials: "include",

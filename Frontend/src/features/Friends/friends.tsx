...
const FriendList: React.FC<{ status: FriendCardProps["status"] }> = ({ status }) => {
  ...
  useEffect(() => {
    ...
    const fetchRequest = async () => {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/friends/allsendreq`, { credentials: "include" });
      const data = await res.json();
      setFriends(Array.isArray(data) ? data : []);
    };

    const myAllfriends = async () => {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/friends/allfriends`, { credentials: "include" });
      const data = await res.json();
      setAllUsers(data);
    };

    const allRecvReq = async () => {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/friends/allrecvreq`, { credentials: "include" });
      const data = await res.json();
      setAllreqs(Array.isArray(data) ? data : []);
    };

    const allBlocked = async () => {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/friends/blockReq`, { credentials: "include" });
      const data = await res.json();

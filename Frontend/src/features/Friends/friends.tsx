...
const FriendList: React.FC<{ status: FriendCardProps["status"] }> = ({ status }) => {
  ...
  useEffect(() => {
    const fetchRequest = async () => {
      const res = await fetch("http://e3r1p17.1337.ma:3000/friends/allsendreq", { credentials: "include" });
      const data = await res.json();
      setFriends(Array.isArray(data) ? data : []);
    };

    const myAllfriends = async () => {
      const res = await fetch("http://e3r1p17.1337.ma:3000/friends/allfriends", { credentials: "include" });
      const data = await res.json();
      setAllUsers(data);
    };

    const allRecvReq = async () => {
      const res = await fetch("http://e3r1p17.1337.ma:3000/friends/allrecvreq", { credentials: "include" });
      const data = await res.json();
      setAllreqs(Array.isArray(data) ? data : []);
    };

    const allBlocked = async () => {
      const res = await fetch("http://e3r1p17.1337.ma:3000/friends/blockReq", { credentials: "include" });
      const data = await res.json();
  ...
  const handleAccept = async (username: string) => {
    ...
    await fetch("http://e3r1p17.1337.ma:3000/friends/acceptrequest", {
  ...
  const handleReject = async (username: string) => {
    ...
    await fetch("http://e3r1p17.1337.ma:3000/friends/deletereq", {
  ...
  const unblockUser = async (username: string) => {
    ...
    await fetch("http://e3r1p17.1337.ma:3000/friends/unblockUser", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ frined_username: username, type: "send" }),
      credentials: "include",
      ...
      <div className="sticky top-12 z-20 px-14 pt-4 pb-4 bg-[#393E46]">
        ...
        <div className="flex flex-col ">
            {status === "sent" && friends.length > 0 ? (
                ...
                friends.map(user => (
                <FriendCard key={user.username} name={user.username} image_url={user.image_url} status="sent" onCancel={handleCancel} />
                ))

            ) : status === "all" && allUsers.length > 0 ? (

                allUsers.map(user => (
                <FriendCard key={user.username} name={user.username} image_url={user.image_url} status="all" />
                ))
                // <></>
                ...
                allreqs.map(user => (
                <FriendCard key={user.username} name={user.username} image_url={user.image_url} status="request" onAccept={handleAccept} onReject={handleReject} />
                ))

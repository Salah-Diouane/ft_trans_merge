...
const FriendList: React.FC<{ status: FriendCardProps["status"] }> = ({
  status,
}) => {
  ...
  useEffect(() => {
    ...
    if (status === "sent") fetchRequest();
    if (status === "all") myAllfriends();
    if (status === "request") allRecvReq();
    if (status === "blocked") allBlocked();
  }, [status, dataUpdate]);

    ...
    <div className="flex flex-col w-full h-full  max-sm:h-[100%] rounded-2xl p-[2px] max-lg:bg-none ">
      <div className="flex flex-col flex-grow  rounded-xl max-lg:rounded-none p-2 overflow-hidden bg-no-repeat bg-cover" style={{ backgroundImage: `url(${Subtract})` }}>
        ...
        <div className="flex items-center justify-between p-3 m-1 bg-[#2f3542] rounded-xl max-lg:rounded-b-lg max-lg:h-20 max-lg:m-[-4px] border-b-2 border-[#2f3542]">
          ...
          <Threedots user={user} loggedInUserId={loggedInUserId} isBlocked={isBlocked} setAllBlocked={setAllBlocked} setShowInvBlock={setShowInvBlock} setShowInvBlockmenu={setShowInvBlockmenu} showInvBlock={showInvBlock} />
        </div>

        <Messages messages={messages} loggedInUserId={loggedInUserId} handleThreeDotsClick={handleThreeDotsClick} showMenu={showMenu} setShowMenu={setShowMenu} handleDeleteClick={handleDeleteClick} />
        <DeleteMessage confirmDelete={confirmDelete} cancelDelete={cancelDelete} showDeleteModal={showDeleteModal} />
        <BlockUser showInvBlockMenu={showInvBlockMenu} confirmBlock={confirmBlock} cancelBlock={cancelBlock}/>


        {!isBlocked ? (
            <InputSend input={input} setInput={setInput} isBlocked={isBlocked} onSend={onSend} showEmojiPicker={showEmojiPicker} setShowEmojiPicker={setShowEmojiPicker}/>
        ) : (
            <InputBlocked input={input} setInput={setInput} onSend={onSend} isBlocked={isBlocked}/>
        )}
      </div>
    </div>
  );

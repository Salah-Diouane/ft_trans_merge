...
export default function GameStats({ type , username}: { type: string , username: string}) {

    const [GameSates, setGameSates] = useState<game_state>({ win: 0, lose: 0, draw: 0 });
    const [rank, setRank] = useState(0);
    useEffect(() => {
        const getGameSate = async () => {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/profile/GameStats/${username}`, {
                method: 'GET',
                credentials: 'include',
            });
            if (response.ok) {
                const GameSate = await response.json() as { gamestate: game_state, rank: number };
    ...
    return (type === 'Doughnut' ? <GamestateDoughnut win={wins} lose={losses} draw={draws} />

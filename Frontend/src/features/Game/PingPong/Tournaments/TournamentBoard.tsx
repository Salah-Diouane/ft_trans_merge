...
const TournamentBoard: React.FC<TournamentBoardProps> = ({ onStartNextRound }) => {
  ...
  const getUsernameFromId = (id: string): string => {
    console.log('Mapping ID to username:', id, idToUsernameMap[id]);
  ...
  const fetchTournament = useCallback(async () => {
    if (!tournamentId || fetchingRef.current) {
      console.log('Fetch prevented:', { tournamentId, fetching: fetchingRef.current });
      return;
    }
    ...
    try {
      console.log(`🔄 Fetching tournament ${tournamentId} (attempt ${retryCount + 1}/${MAX_RETRIES + 1})`);
      ...
      console.log("✅ Tournament fetched successfully:", data.tournament.id);
  ...
  useEffect(() => {
    ...
    console.log(`Joining tournament room: tournament:${tournamentId}`);
    ...
    const handleTournamentUpdated = async (updatedTournament: Tournament) => {
      ...
      if (updatedTournament.id === tournamentId) {
        console.log('Tournament updated via socket:', updatedTournament);
    ...
    socket.on('readyToPlay', (payload) => {
      console.log('Ready to play event received:', payload);
    ...
    return () => {
      console.log(`Leaving tournament room: tournament:${tournamentId}`);
  ...
  const isOwner = userId === ownerName;

...
export default function PlayerLevel({ username }: { username: string }) {
	...
	useEffect(() => {
		const getData = async () => {
			try {
				const response = await fetch(`${import.meta.env.VITE_API_URL}/api/profile/UserPlayerStats/${username}`,
					{
						credentials: 'include',
						method: 'GET',
					}
	...
	return (
		<div className="w-[280px]  mx-auto">
			<div className="w-full rounded-full h-5 shadow-inner overflow-hidden border border-gray-700">
				<motion.div className="h-full rounded-full bg-blue-500" initial={{ width: 0 }} animate={{ width: `${levelData.percent}%` }} transition={{ duration: 2, ease: 'easeOut' }} />
			</div>
			<motion.p className="text-center text-[11px] text-gray-300 mt-2 tracking-wide font-mono" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}>Level{' '}<span className="text-blue-400 font-semibold">

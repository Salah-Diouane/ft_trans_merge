...
// 		const firstError = error.validation[0];
// 		const field = firstError.instancePath.replace(/^\//, "") || "field";
// 		const keyword = firstError.keyword; // e.g., minLength, pattern, maxLength
// 		const lang = user?.Language || "en";

// 		console.log('lang L')
// 		const message =
// 			validationMessages[field]?.[keyword]?.[lang] ||
// 			validationMessages[field]?.[keyword]?.["en"] ||
// 			firstError.message ||
// 			"Validation error";
// 		return reply.status(400).send({ type: field, message: message });
// 	}
// });

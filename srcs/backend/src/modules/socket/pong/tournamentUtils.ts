...
export function handlePlayerLeft(fastify:FastifyInstance, io: IOServer, roomId: string, playerName: string | undefined, tournament: any, game: any) {
	console.log("Handling player left event for", playerName);

	console.log("EMIT PLAYER LEFT", playerName);
	...
	if (tournament) {
		...
		if (match && match.progress !== 'completed') {
			...
			handlePlayerForfeit(
				...
				remainingPlayer,
				io);
			console.log("here1", match.winners, match.losers);
			addNewHistory(
				fastify,
			...
			cleanupGame(roomId);
			console.log("Deleting game for room:", roomId, Object.keys(games).length);
...
export function handleTournamentPlayerLeft(fastify:FastifyInstance, io: IOServer, roomId: string, game: any, playerName: string, remainingPlayer: string | undefined) {
	console.log(`Tournament match: ${playerName} left, ${remainingPlayer || 'no one'} wins by default`);
	...
	console.log("here1", tournaments, roomId, game.winner, game.loser);

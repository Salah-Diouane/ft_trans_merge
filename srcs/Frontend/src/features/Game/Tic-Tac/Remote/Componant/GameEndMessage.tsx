...
const GameEndMessage: React.FC<{
  gameResult: GameResult | null;
  startNewGame: () => void;
}> = ({ gameResult, startNewGame }) => {
  if (!gameResult) return null;

  const { reason, winner, loser } = gameResult;

  let message;
  if (reason === "draw") message = "It's a Draw!";
  else if (reason === "timeout") message = `${loser} ran out of time!`;
  else if (reason === "disconnect") message = `${loser} disconnected!`;

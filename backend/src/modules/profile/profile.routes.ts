...
import { getUserGameStats, getPlayerState, PlayerState, getPlayerHistory, countGames } from '../../utils/profile.utils';
...
export const UserGameStats = async (fastify: FastifyInstance) => {
	fastify.get('/GameStats/:username?', async (request: FastifyRequest<{ Params: { username?: string } }>, reply) => {
		try {
...
export const PlayerHistory = async (fastify: FastifyInstance) => {
	fastify.get('/PlayerHistory/:username?', async (request: FastifyRequest<{ Params: { username?: string } }>, reply) => {
		try {
			const { username } = request.params;
			let userid: number | null = null;
			if (!username) {
...
export const Userinfo = async (fastify: FastifyInstance) => {
	fastify.get('/userinfo/:username?', async (request, reply) => {
		try {
			...
			if (!username)
				return reply.code(400).send({ userinfo: false, message: "Username is required" });
			const user = await getuser(fastify, username);
			if (!user)
				return reply.code(404).send({ userinfo: false, message: "User not found" });
			return reply.send({
				userinfo: true,
				data: {
			...
			reply.code(500).send({ userinfo: false, error: "Internal server error" });
		}
	});
}

export const CountGames = async (fastify: FastifyInstance) => {
	fastify.get('/CountGames/:username?', async (request: FastifyRequest<{ Params: { username?: string } }>, reply) => {
		try {
			const { username } = request.params;
			let userid: number | null = null;
			if (!username) {
